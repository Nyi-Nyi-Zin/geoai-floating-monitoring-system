...
Native inspection of the acquired 12-event GFD archive set found EPSG:4326, five-band TIFFs with an approximately **0.0022458°** pixel step. Event raster bounding boxes intersected between 17 and 18 Admin 1 bounding boxes, which confirms country-scale geographic coverage but intentionally **overstates** real overlap. The inspection read metadata only; it did not sample flood pixels, make labels, construct features, fit a model, or create any nationwide risk output. True-polygon masking and per-event coverage checks remain prerequisites for regional label generation.
